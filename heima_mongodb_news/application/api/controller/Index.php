...
class Index extends Controller
{
    ...
    public function read($id)
    {
        ...
        $db = getMongoDB();
        $table = $db->news;
        $one = $table->findOne(['_id' => new \MongoDB\BSON\ObjectID($id)]);
        $table->updateOne(
        ...
        // $one = $one->toArray();
        $comments = [];
        $comments = $db->comment->find(['news_id' => $id]);
    ...
    public function login($username,$password){
        ...
        $db = getMongoDB();
        $table = $db->client;
        $result = $table->findOne(['username'=>$username,'password'=>$password]);
        // var_dump($res);
        if($result){
            $response = [
                'code' => '200',
    ...
    public function addcomment(){
        ...
        $db = getMongoDB();
        $table = $db->comment;
        $exist = $table->findOne(['news_id'=>$news_id,'username'=>$username]);
        if($exist){
            $response = [
                'code' => '000',
                'msg' => '一个用户只能对一篇文章评论一次'
